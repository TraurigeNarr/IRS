...
void CPropertyGrid::SetTrueFalseStrings(std::wstring strTrue, std::wstring strFalse)
{
  m_strTrue = strTrue;
  m_strFalse = strFalse;
}

void CPropertyGrid::SetOkCancelStrings(std::wstring strOk, std::wstring strCancel)
{
  m_strOk = strOk;
  m_strCancel = strCancel;
}

void CPropertyGrid::SetDateTimeStrings(std::wstring strDate, std::wstring strTime)
{
  m_strDate = strDate;
  m_strTime = strTime;
}

void CPropertyGrid::SetUndefinedString(std::wstring strUndefined)
{
  m_strUndefined = strUndefined;
}

void CPropertyGrid::SetEmptyString(std::wstring strEmpty)
{
  m_strEmpty = strEmpty;
}
...
bool CPropertyGrid::CItem::operator==(const std::wstring& name) const
{
  return m_name == name;
}
...
HSECTION CPropertyGrid::AddSection(std::wstring title, bool collapsed, HSECTION after)
{
  // build it
  CSection section;
...
HITEM CPropertyGrid::AddItem(HSECTION hs, EItemType type, std::wstring name, void* pValue, bool editable, bool undefined, HITEM after)
{
  ...
  else if (type == IT_STRING || type == IT_TEXT || type == IT_FILE || type == IT_FOLDER) item.m_strValue = *(std::wstring*)pValue;
...
HITEM CPropertyGrid::AddCustomItem(HSECTION section, std::wstring name, ICustomItem* pItem, bool editable, HITEM after)
{
  pItem->m_pGrid = this;
  return AddItem(section, IT_CUSTOM, name, pItem, editable, false, after);
}

HITEM CPropertyGrid::AddStringItem(HSECTION section, std::wstring name, std::wstring value, bool editable, HITEM after)
{
  return AddItem(section, IT_STRING, name, &value, editable, false, after);
}

HITEM CPropertyGrid::AddTextItem(HSECTION section, std::wstring name, std::wstring value, bool editable, HITEM after)
{
  return AddItem(section, IT_TEXT, name, &value, editable, false, after);
}

HITEM CPropertyGrid::AddIntegerItem(HSECTION section, std::wstring name, int value, std::wstring format, bool editable, bool undefined, HITEM after)
{
...
HITEM CPropertyGrid::AddDoubleItem(HSECTION section, std::wstring name, double value, std::wstring format, bool editable, bool undefined, HITEM after)
{
...
HITEM CPropertyGrid::AddComboItem(HSECTION section, std::wstring name, const vector<std::wstring>& values, int cur, bool editable, bool undefined, HITEM after)
{
...
HITEM CPropertyGrid::AddBoolItem(HSECTION section, std::wstring name, bool value, bool editable, bool undefined, HITEM after)
{
  return AddItem(section, IT_BOOLEAN, name, &value, editable, undefined, after);
}

/*
HITEM CPropertyGrid::AddDateItem(HSECTION section, std::wstring name, COleDateTime value, std::wstring format, bool editable, bool undefined, HITEM after)
{
...
HITEM CPropertyGrid::AddDateTimeItem(HSECTION section, std::wstring name, COleDateTime value, std::wstring format, bool editable, bool undefined, HITEM after)
{
...
HITEM CPropertyGrid::AddFileItem(HSECTION section, std::wstring name, std::wstring value, std::wstring filter, bool editable, HITEM after)
{
...
HITEM CPropertyGrid::AddFolderItem(HSECTION section, std::wstring name, std::wstring value, std::wstring title, bool editable, HITEM after)
{
...
HITEM CPropertyGrid::AddColorItem(HSECTION section, std::wstring name, COLORREF value, bool editable, bool undefined, HITEM after)
{
  return AddItem(section, IT_COLOR, name, &value, editable, undefined, after);
}

HITEM CPropertyGrid::AddFontItem(HSECTION section, std::wstring name, LOGFONT value, bool editable, bool undefined, HITEM after)
{
...
bool CPropertyGrid::GetItemValue(HITEM item, std::wstring& strValue) const
{
  // get the item
...
bool CPropertyGrid::SetItemValue(HITEM item, const std::wstring strValue)
{
  // get the item
  CItem* pItem = FindItem(item);
  if (pItem == NULL) return false;
...
void CPropertyGrid::DrawItem(CDC& dc, int w, int x, int y, vector<CItem>::iterator& it)
{
  ...
  else
  {
    ...
    // now draw text
    std::wstring strValue = it->m_strValue;
    switch (it->m_type)
    {
    case IT_TEXT:
      {
        size_t j;
        for (;( j = strValue.find( L"\r\n" )) != std::wstring::npos;)
          strValue.replace( j, 2, L"¶");
        break;
      }
    ...
    case IT_COLOR:
      {
        ...
        // update the text
        CString strTemp;
        strTemp.Format(L"%d; %d; %d", GetRValue(it->m_clrValue), GetGValue(it->m_clrValue), GetBValue(it->m_clrValue));
        strValue = strTemp.GetString();
    ...
    case IT_FONT:
      {
        CString strTemp;
        strTemp.Format(L"%s; %dpt", it->m_lfValue.lfFaceName, -MulDiv(it->m_lfValue.lfHeight, 72, dc.GetDeviceCaps(LOGPIXELSY)));
        strValue = strTemp.GetString();
      ...
      if (pItem)
      {
        if (GetEditMode(*pItem) == EM_DROPDOWN)
        {
          ...
          else
          {
            ...
            BOOL result = pCombo->Create(WS_POPUP, rc, this, 0);
            ...
            {
              for (std::vector<std::wstring>::iterator it = pItem->m_options.begin(); it != pItem->m_options.end(); ++it)
                pCombo->AddString(*it);
              if (!pItem->m_undefined)
        ...
        else if (GetEditMode(*pItem) == EM_INPLACE)
        {
          ...
          else if (pItem->m_type == IT_INTEGER)
          {
            ...
            {
              CString strTemp;
              strTemp.Format(L"%d", pItem->m_nValue);
              strValue = LPCTSTR(strTemp);
            }
          }
          else if (pItem->m_type == IT_DOUBLE)
          {
            ...
            {
              CString strTemp;
              strTemp.Format(L"%g", pItem->m_dValue);
              strValue = LPCTSTR(strTemp);
            }
          }
...
LRESULT CPropertyGrid::OnEditChanged(WPARAM wParam, LPARAM lParam)
{
  ...
  if (pItem)
  {
    if (pItem->m_type == IT_STRING)
    {
      pItem->m_strValue = std::wstring((wchar_t*)wParam);
      pItem->m_undefined = false;
      GetOwner()->SendMessage(WM_PG_ITEMCHANGED, pItem->m_id);
      DeleteEditControl();
      Invalidate();
    ...
    {
      if (wcslen((wchar_t*)wParam))
      {
        pItem->m_nValue = _wtoi((wchar_t*)wParam);
        pItem->m_undefined = false;
        GetOwner()->SendMessage(WM_PG_ITEMCHANGED, pItem->m_id);
      }
      DeleteEditControl();
    ...
    else if (pItem->m_type == IT_DOUBLE)
    {
      if (wcslen((wchar_t*)wParam))
      {
        pItem->m_dValue = _wtof((wchar_t*)wParam);
        pItem->m_undefined = false;
        GetOwner()->SendMessage(WM_PG_ITEMCHANGED, pItem->m_id);
      }
      DeleteEditControl();
    ...
    {
      if (pItem->m_pCustom->OnItemEdited(std::wstring((wchar_t*)wParam)))
        GetOwner()->SendMessage(WM_PG_ITEMCHANGED, pItem->m_id);
      DeleteEditControl();
      Invalidate();
    }
