...
void CPropertyGridCombo::AddString(const std::wstring& strItem)
{
...
BOOL CPropertyGridCombo::Create(DWORD dwStyle, CRect& rc, CWnd* pParent, int nId)
{
  ...
  BOOL ret = __super::CreateEx(0, AfxRegisterWndClass(CS_HREDRAW|CS_VREDRAW), L"", dwStyle, rc, pParent->GetParent(), nId);
...
void CPropertyGridCombo::OnPaint()
{
  ...
  for (vector<wstring>::iterator it = m_Items.begin(); it != m_Items.end(); ++it)
  {
